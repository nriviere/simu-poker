...
double IAPlayer::getProba(char *hand, int nbPlayer){
	...
	//On cr�e un deuxieme tableau o� les cartes sont invers�es car les cartes n'ont pas d'ordre dans le fichier
	...
	if(probaFile.is_open()){
		...
		while(!found){
			if(!probaFile.eof()){ //tant que la fin du fichier n'est pas atteint.
				probaFile >> handproba;
				if(strcmp(hand,handproba.c_str()) == 0 || strcmp(inverseHand,handproba.c_str()) == 0){ // la main est trouv�e
					found = true;
				}else{
					getline(probaFile,dump); // ce n'est pas la bonne main, on jette la ligne.
				}
			}else{ //la fin du fichier est atteint sans avoir trouv� de main identique : erreur, on s'arrete.
		...
		if(found){
			if(nbPlayer < 1){
				cerr << "le nombre d'adversaire ne peut �tre nul : calcul des probas pour 1 adversaire..." << endl;
				nbPlayer = 1;
			}else if (nbPlayer > 9){
				cerr << "le nombre d'adversaire ne peut �tre sup�rieur a 9 : calcul des probas pour 9 adversaires..." << endl;
				nbPlayer = 9;
			}
